...
class EnsembleModel(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.kd_omega1 = args.KD_omega1
        self.kd_omega2 = args.KD_omega2
        ...
        if args.pretained == True:
            self.global_model.load_state_dict(torch.load(os.path.join("/home/test/lrz/kge/11_29", args.dataset, f"{args.global_model}_rank{args.global_model_rank}", "model.pt")), strict=False)
            # self.local_model.load_state_dict(torch.load(os.path.join("/home/test/lrz/kge/11_29", args.dataset, f"{args.local_model}_rank{args.local_model_rank}", "model.pt")), strict=False)
    ...
    def mutual_learning(self, score_t, score_s):
        ...
        return self.kd_omega1 * distill_loss + (1.0 - self.kd_omega1) * neg_dist_loss
    
    def neg_dist_learning(self, neg_score_s, neg_score_t):
        neg_score_s_ = F.softmax(-neg_score_s, dim=1).clone().detach() * neg_score_s
        neg_score_t_ = F.softmax(-neg_score_t, dim=1).clone().detach() * neg_score_t
    ...
    def feature_learning(self, pos_samples, neg_samples, model_t: BaseH, model_s: BaseH):
        ...
        pos_d_stru = self.kd_omega2 * pos_d_angle + (1.0 - self.kd_omega2) * pos_d_lr
        neg_d_stru = self.kd_omega2 * neg_d_angle + (1.0 - self.kd_omega2) * neg_d_lr
